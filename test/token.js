...
contract("Token", function(accounts) {
  // CREATION
  it("creation: should create an initial balance of 260000000000000 for the creator", function(
    done
  ) {
    Token.new({ from: accounts[0] })
      ...
      .then(function(result) {
        assert.strictEqual(result.toNumber(), 260000000000000);
        done();
      })
      .catch(done);
  });
  ...
  // TRANSERS
  it("transfers: should transfer 260000000000000 to accounts[1] with accounts[0] having 1000000", function(
    done
  ) {
    var ctr;
    Token.new({ from: accounts[0] })
      .then(function(result) {
        ctr = result;
        return ctr.transfer(accounts[1], 260000000000000, {
          from: accounts[0]
        });
      })
      .then(function(result) {
  ...
  it("transfers: should fail when trying to transfer 260000000000001 to accounts[1] with accounts[0] having 1000000", function(
    done
  ) {
    var ctr;
    Token.new({ from: accounts[0] })
      .then(function(result) {
        ctr = result;
        return ctr.transfer.call(accounts[1], 260000000000001, {
          from: accounts[0]
        });
      })
      .then(function(result) {
  ...
  it("approvals: msg.sender approves accounts[1] of 100 & withdraws 20 once.", function(
    ...
    Token.new({ from: accounts[0] })
      ...
      .then(function(result) {
        assert.strictEqual(result.toNumber(), 259999999999980);
        done();
      })
      .catch(done);
  });
  ...
  it("approvals: msg.sender approves accounts[1] of 100 & withdraws 20 twice.", function(
    ...
    Token.new({ from: accounts[0] })
      ...
      .then(function(result) {
        assert.strictEqual(result.toNumber(), 259999999999960);
        done();
      })
      .catch(done);
  });
  ...
  it("approvals: msg.sender approves accounts[1] of 100 & withdraws 50 & 60 (2nd tx should fail)", function(
    ...
    Token.new({ from: accounts[0] })
      ...
      .then(function(result) {
        assert.strictEqual(result.toNumber(), 259999999999950);
